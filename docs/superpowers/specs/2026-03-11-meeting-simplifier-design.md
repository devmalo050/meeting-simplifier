...
회의록 본문을 안건별 논의 흐름·구조화된 액션/결정·부가 섹션으로 상세화.

- `stop.md` 템플릿 교체: `한 줄 요약` + `핵심 포인트` + `논의 내용`(안건별 *논의/의견·이견/결론*) + `결정 사항`(근거) + `액션 아이템`(담당·기한 체크박스) + `미결 사항`/`다음 단계`, 메타(녹음 길이·참석자·키워드).
- **환각 방지:** 음성에 없는 정보(담당·기한·참석자·근거)는 생략, 내용 없는 섹션은 통째로 생략(`한 줄 요약`·`핵심 포인트`·`논의 내용`만 항상 포함).
- 화자 분리(diarization)는 비목표 — 참석자는 음성에 이름이 언급된 경우만.
- 코드 변경 없음(프롬프트 템플릿만). 상세 설계: `docs/superpowers/specs/2026-06-02-meeting-minutes-detail-design.md`.
